...
TOKEN = 'MTIyMTc4NzIyNDQ5NzA2NjAyNg.GBuJU3.VQPHdHK4MGYfcWk0qpgFni4Y3oyMSYA5SoDn7c'
...
async def on_ready():
    print("Bot is ready")
    await tree.sync()
    await client.change_presence(status=discord.Status.dnd, activity=discord.Game("ScriptLuaVip 🔥"))
...
async def arceux(interaction: discord.Interaction, link: str):
    ...
    if not hwid:
        ...
        embed.set_footer(text="Made by ScriptLuaVip")
    ...
    if response.status_code == 200:
        ...
        if 'key' in data:
            if data['key'] == 'Error Pls New Link or Try again':
                ...
                embed.set_footer(text="Made by ScriptLuaVip")
                embed.set_thumbnail(url="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQDXWPxWgfrFsPT9M9NzG2PLeMg3nWE5LkAIw&s")
        else:
            embed = discord.Embed(
                title="**Error**",
                description="**Unexpected response from API**",
